...
/* See lib/db.js. The seeds and the candidates are matched against each other
   by artist, so both sides have to read the name the same way — a profile
   built from raw tags could never match a candidate the user has renamed. */
...
function buildProfile(db, now) {
  ...
  const seeds = db.prepare(
    `SELECT id, ${NAME.title} AS title, ${NAME.artist} AS artist, genre, year, last_played_at
     FROM albums
...
function build(db, now = Date.now()) {
  ...
  const candidates = db.prepare(
    `SELECT ${ALBUM_COLS}, ${NAME.sortArtist} AS sort_artist, last_played_at AS lp
     FROM albums
     WHERE present = 1
