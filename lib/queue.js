...
/* Straight from the protocol, never through lib/sonos.js — two import paths for
   one function is how a partial migration hides. */
const { hmsToSeconds } = require("./upnp");
...
/*
 * How long a track counts as JUST STARTED, for the back button.
 *
 * Pressed after this, Back restarts the track you are on; pressed inside it,
 * it goes to the one before. Everything with a back button works this way —
 * Sonos's own Previous does it on the speaker, which is why `SonosQueue`
 * delegates rather than reimplementing it, and LMS's rew does the same thing
 * with a double press ("either starts the same song over, or the previous one",
 * Slim/Buttons/Common.pm). It falls out of the same rule here: a restart puts
 * the track back to zero, so a SECOND press lands inside the window and moves
 * back a track.
 */
const JUST_STARTED_SECONDS = 3;

/*
 * How long a device is allowed to take to open a URI it has just been handed.
 *
 * Not a timeout — the SOAP call has already been answered. This is how long
 * `playing()` keeps preferring what the server just SET over what the device
 * still REPORTS, because a renderer opening a new stream carries on describing
 * the track before it for a second or two. Long enough to cover that and the
 * poll behind it; short enough that a device somebody sent elsewhere from its
 * own app is believed almost at once.
 */
 ...
 *   playing(pos)    -> { uri, seconds, duration }: what is coming out of the
 *                      speakers NOW, from the position info the poll already
 *                      read. Not simply `pos` itself — see ServerQueue, where
 *                      the device lags what it has been told. A zero duration
 *                      means the device has not got one, and the caller uses
 *                      what the library knows.
 *   next()          -> the track after this one.
 *   previous()      -> BACK, which is "start this again" once you are past
 *                      JUST_STARTED_SECONDS and "the one before" inside it.
 *                      Not the mirror of next(), and no implementation of a
 *                      back button anywhere is.
 *
 * NEXT AND PREVIOUS BELONG HERE, not on the player, however much they look
 * like transport. They are only meaningful against a LIST: AVTransport's Next
 * and Previous move through the queue the DEVICE holds, and a stock renderer
 * has none — it is playing one URI and there is nothing to move to. Sending
 * them anyway is a button that does nothing, which is what shipped in 0.4.47.
 *
...
class SonosQueue {
  ...
  /* The speaker moved its own queue, in one action, before it answered. There
     is no window in which it is playing one thing and reporting another. */
  playing(pos) {
    return {
      uri: (pos && pos.uri) || "",
      seconds: hmsToSeconds(pos && pos.relTime),
      duration: hmsToSeconds(pos && pos.duration)
    };
  }

  /* The speaker moves through its own queue, and its Previous already does the
     JUST_STARTED_SECONDS thing people expect from the button — in the firmware,
     matched to the Sonos app, tested against every room they sell.
     Reimplementing either here would be worse than both. */
  next() { return this.player.next(); }
  previous() { return this.player.previous(); }
}
...
class ServerQueue {
   ...
   */
  constructor(player, { db, zoneId, entryFor, sent = new Map() }) {
    this.player = player;
    this.db = db;
    this.zoneId = zoneId;
    /*
     * WHAT EACH ROOM'S DEVICE WAS LAST HANDED, AND WHEN — see playing().
     *
     * PASSED IN, because `queueFor()` builds one of these per call and a fact
     * about a window of seconds would be forgotten the moment it was recorded.
     * A Map keyed by zone rather than a column: it describes a device catching
     * up, and a server that restarted has no press left to catch up with.
     */
    this.sentByZone = sent;
    /*
  ...
  get sent() { return this.sentByZone.get(this.zoneId) || null; }
  /* Was this device handed something so recently that it may not have started
     it yet? Asked by anything that would otherwise read a silent device as a
     device that has stopped. */
  get settling() {
    const sent = this.sent;
    return !!sent && Date.now() - sent.at <= SETTLING_MS;
  }
  set sent(v) {
    if (v) this.sentByZone.set(this.zoneId, v);
    else this.sentByZone.delete(this.zoneId);
  }
  ...
  /*
   * WHAT IS PLAYING, WHICH THE DEVICE IS THE LAST TO KNOW.
   *
   * A renderer handed a new URI carries on describing the track BEFORE it while
   * it opens the stream — a second or two on a WiiM. Believing that answer
   * paints the previous track, still advancing, for as long as the device takes,
   * which reads as the screen ignoring the button: press skip near the end of a
   * record and the old waveform sits there fully played before the new one
   * appears. Sonos has no such window, because its own Next moves the queue
   * inside the speaker before it answers — which is exactly why this was only
   * ever reported here.
   *
   * So while the device is still reporting something other than what it was
   * last told, the answer is what it was told, at the top. It self-clears the
   * moment the device agrees, and expires anyway, so a device sent elsewhere
   * from its own app is believed rather than argued with.
   */
  playing(pos) {
    const said = {
      uri: (pos && pos.uri) || "",
    ...
    const sent = this.sent;
    if (!sent || Date.now() - sent.at > SETTLING_MS) return said;

    /*
     * IT HAS ARRIVED WHEN IT REPORTS WHAT IT WAS ASKED FOR, which is that URI
     * AT THE TOP — not the URI alone. Restarting a track does not change the
     * URI at all, so a URI test alone calls a restart settled on the first
     * poll and hands back the position from before the press: press back near
     * the end of a track and the bar stays at 98% for a second before dropping.
     * That is the same report as the skip one, wearing different clothes.
     */
    if (said.uri === sent.uri && said.seconds <= JUST_STARTED_SECONDS) {
      this.sent = null;
      return said;
    }
    /*
     * NOT CAUGHT UP IS SAYING THE TRACK IT WAS ON BEFORE, or that track again.
     * Anything else is a device that genuinely went somewhere — sent by its own
     * app — and is believed AT ONCE: without that this window would argue with
     * a redirect for five seconds, which is the opposite of the rule that the
     * room follows what is playing rather than a counter.
     */
    if (said.uri !== sent.uri && said.uri !== sent.from) { this.sent = null; return said; }
    /* No duration either: the device's is still the track that has just ended,
       and a length from one track against a position from another is a bar
       that jumps. The caller falls back to what the library knows. */
    return { uri: sent.uri, seconds: 0, duration: 0 };
  }
  ...
  /*
   * ONE TRACK ALONG, WHICHEVER WAY.
   *
   * AVTransport's own Next and Previous move through the queue the DEVICE
   * holds, and this device holds none — it is playing one URI, with nothing to
   * move to. Sending them is a button that does nothing, which is exactly what
   * 0.4.47 shipped. The queue that actually exists is here, so the move is
   * made here and the device is simply pointed at the result.
   *
   * The ends are said in words rather than left as "nothing is queued at
   * position 0", which is true and unhelpful.
   */
  async next() {
    const at = this.at();
    if (!this.trackAt(at + 1)) throw new Error("That is the last track in the queue.");
    return this.playFrom(at + 1, true);
  }

  /*
   * BACK IS "START THIS AGAIN" BEFORE IT IS "THE ONE BEFORE".
   *
   * 0.4.49 read it as the mirror of next() and refused at the top of the
   * queue — so deciding to hear the first track of a record from the beginning
   * got an error, which is the one thing the button was being pressed for.
   * Nothing works that way: a speaker's own Previous restarts the track once
   * you are into it, and so does every remote made.
   *
   * The device is asked how far in it is, which costs a round trip on a press
   * somebody is waiting for — the right side of that trade, and the same one
   * the cover picker makes. A device that will not say is taken as "well into
   * it": restarting a track you meant to leave costs one more press, where
   * leaving a track you meant to restart loses your place.
   */
  async previous() {
    const at = this.at();
    if (at > 1 && await this.justStarted()) return this.playFrom(at - 1, true);
    return this.playFrom(at, true);
  }

  /** Are we inside the opening seconds of the current track? */
  async justStarted() {
    try {
      const pos = await this.player.positionInfo();
      return hmsToSeconds(pos.relTime) < JUST_STARTED_SECONDS;
    } catch {
      /* Not silence for its own sake: an unanswered position is the "well into
         it" case above, and the reason is stated there rather than twice. */
      return false;
    }
  }
  ...
  async playFrom(position, autoplay) {
    ...
    /* Read BEFORE setAt moves it: `from` is the track being LEFT. */
    const from = this.uriFor(this.trackAt(this.at()));
    this.setAt(position);
    const uri = this.uriFor(trackId);
    /* What the device has been TOLD to play and what it was playing before, so
       playing() can tell "has not caught up yet" from "is somewhere else". */
    this.sent = { uri, from, at: Date.now() };
    await this.player.setAvTransportUri(uri, this.metadataFor(trackId));
    if (autoplay) await this.player.play();
