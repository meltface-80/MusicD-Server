 ...
 *   next() / previous() -> one track along, either way.
 *
...
class SonosQueue {
  ...
  /* The speaker moves through its own queue, and its Previous already does the
     "restart this track if you are well into it" thing people expect from the
     button. Reimplementing either here would be worse than both. */
...
class ServerQueue {
  ...
  async previous() {
    const at = this.at();
    if (at <= 1) throw new Error("That is the first track in the queue.");
    return this.playFrom(at - 1, true);
  }
