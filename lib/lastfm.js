...
function createLastfm({ db, settings, apiKey = "", apiSecret = "", fetchImpl = null }) {
  ...
  /* ---------------------------------------------------------------- */
  /*  Reading, for lib/info.js                                         */
  /* ---------------------------------------------------------------- */

  /*
   * The two calls that only ever READ, and the only two here that need no
   * signature — they change nothing, so Last.fm asks for the api_key alone.
   *
   * They exist for lib/info.js, which uses Last.fm as the fallback source for
   * a record Wikipedia has never heard of. Nothing about scrobbling depends on
   * them, and an account does not have to be connected for them to work: the
   * developer key is enough, which is why they sit outside the session
   * machinery below.
   *
   * autocorrect is ON. It is what turns "Bjork" into "Björk" at Last.fm's end
   * rather than ours — but it also means what comes back may be about a
   * DIFFERENT act than the one asked for, so info.js checks the artist on the
   * way out. A correction is useful; a silent substitution is not.
   */
  function describeArtist(artist) {
    return call("artist.getInfo", { artist: String(artist || ""), autocorrect: "1" });
  }

  function describeAlbum(album, artist) {
    return call("album.getInfo", {
      album: String(album || ""), artist: String(artist || ""), autocorrect: "1"
    });
  }
