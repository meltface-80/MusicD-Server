...
function regroup(db) {
  /*
   * THE RAW COLUMNS, NOT THE NAME ON SCREEN.
   *
   * Everywhere else asks for an album's name through albumNames() in
   * lib/db.js, so a correction the user typed is what gets shown, sorted,
   * searched and played. Grouping is the exception, deliberately: it is a
   * match on what is ON DISK, and a match here MOVES the play counts of the
   * album it demotes onto the primary. A rename that quietly folded two
   * records together would take a history with it, and typing the name back
   * would not bring it back — a bad match costs an album, a missed one costs
   * a duplicate.
   */
