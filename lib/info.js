...
const { albumNames, albumMbid, headAlbum } = require("./db");

const WIKI_API = "https://en.wikipedia.org/w/api.php";
/*
 * Wikidata, which is where MusicBrainz points these days.
 *
 * It used to store a `wikipedia` relation on a release group and now stores a
 * `wikidata` one, so both are read: the direct link when it is there, and the
 * Q-number resolved to its English sitelink when it is not. Same Action API,
 * same house, no key.
 */
...
/*
 * The opening of an article: everything before its first heading.
 *
 * The search path gets this for free — TextExtracts returns an intro-only
 * extract with the candidates — but the id path already has the WHOLE article
 * in hand and asking again for the first paragraph of it would be a second
 * request for something already downloaded.
 */
function intro(text) {
  const out = [];
 ...
 *   apiRoot     ditto
 */
function createInfo({ db, version, lastfm = null, available = true, covers = null,
                      fetchImpl = null, apiRoot = WIKI_API,
                      wikidataRoot = WIKIDATA_API, gapMs = GAP_MS } = {}) {
  ...
  async function ask(params, root = apiRoot) {
    await wait();
    ...
    let res;
    try {
      res = await http(root + "?" + query.toString(), {
  ...
  /*
   * THE ARTICLE AN ID NAMES, rather than the one a search guesses at.
   *
   * This is the whole reason identification is worth having twice over. A
   * search always answers — Wikipedia ranks the disambiguation page for "Hex"
   * above the Bark Psychosis album, Earth made a record of the same name — so
   * everything found by searching has to be checked against the library's own
   * facts before it is believed, and an album whose tags are wrong has no
   * facts worth checking against. Those are exactly the albums that end up
   * with no write-up at all.
   *
   * A confirmed release has none of that trouble. MusicBrainz says which
   * record the pressing belongs to, the record says which encyclopaedia entry
   * it is, and there is nothing to verify because nothing was guessed. NO
   * artist check here on purpose: it exists to catch a bad guess, and this
   * path does not guess.
   *
   * Two requests to MusicBrainz through lib/covers.js's gate — the one this
   * application has — then at most one to Wikidata and one to Wikipedia. Once
   * per album, ever: a hit is kept for good.
   */
  async function articleFromRelations(rels) {
    const list = (rels && rels.relations) || [];
    for (const rel of list) {
      const url = String((rel && rel.url && rel.url.resource) || "");
      if (rel && rel.type === "wikipedia" && /^https?:\/\/en\.wikipedia\.org\/wiki\//.test(url)) {
        return decodeURIComponent(url.split("/wiki/")[1] || "").replace(/_/g, " ");
      }
    }
    /* No direct link: MusicBrainz moved these to Wikidata, so follow the
       Q-number to its English sitelink. */
    for (const rel of list) {
      const url = String((rel && rel.url && rel.url.resource) || "");
      const q = /^https?:\/\/(?:www\.)?wikidata\.org\/(?:wiki|entity)\/(Q\d+)$/.exec(url);
      if (!(rel && rel.type === "wikidata" && q)) continue;
      const body = await ask({
        action: "wbgetentities", ids: q[1], props: "sitelinks", sitefilter: "enwiki"
      }, wikidataRoot);
      const entity = body && body.entities && body.entities[q[1]];
      const link = entity && entity.sitelinks && entity.sitelinks.enwiki;
      if (link && link.title) return String(link.title);
    }
    return null;
  }

  async function wikiByRelease(mbid) {
    if (!covers || !mbid) return null;
    const group = await covers.groupOfRelease(mbid, { urgent: true });
    if (!group) return null;
    const rels = await covers.lookupMusicBrainz("release-group", group, "url-rels");
    const title = await articleFromRelations(rels);
    if (!title) return null;

    /* The whole article in one request, then the opening and the reception cut
       out of it here — see intro(). The search path needs two because its
       candidates arrive intro-only. */
    const body = await fullText(title);
    const summary = clamp(intro(body), MAX_SUMMARY);
  ...
  async function album(albumId) {
    ...
    const row = db.prepare(
      `SELECT ${NAME.title} AS title, ${NAME.artist} AS artist, ${MBID} AS mbid
       FROM albums WHERE id = ?`).get(id);
    if (!row) return null;
    ...
    return once("album:" + id, async () => {
      ...
      try {
         ...
         */
        /*
         * The id first, because it cannot be wrong; the search second, because
         * most albums have no id. A failure on the id path is not an answer
         * about this album — it falls through to the search, which is exactly
         * where this album would have been without an id at all.
         */
        const byId = row.mbid ? await wikiByRelease(row.mbid).catch(() => null) : null;
        const found = byId || await wikiAlbum(row) || await lastfmAlbum(row).catch(() => null);
