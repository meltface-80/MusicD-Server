...
function createIdentify({ db, covers, available = true }) {
  ...
  async function candidatesFor(albumId, title, artist) {
    ...
    for (const release of body.releases || []) {
      ...
      scored.push({
        points,
        /* Only ever a tiebreak — see the sort below. */
        year: Number((/^(\d{4})/.exec(String(release.date || "")) || [])[1]) || 9999,
        id: release.id,
    ...
    /*
     * Best first, and the ORIGINAL first among equals. A record reissued three
     * times has three pressings that score identically — same artist, same
     * title, same track count — and offering a 2023 vinyl repress above the
     * 1999 CD puts the least likely answer at the top of the list. An undated
     * release sorts last rather than first: unknown is not zero.
     */
    scored.sort((a, b) => (b.points - a.points) || (a.year - b.year));
