 ...
 *
 * A NEW table rather than columns on the tracks table, because a waveform is a kilobyte
 * of base64 and the track row is read on every wall, every search and every
 * queue paint — SELECTing it alongside them would carry that byte-for-byte
 * through queries that never look at it.
 *
 * gen, rate, size and mtime are what make a stored row TRUSTWORTHY rather than
 * merely present. A waveform is only comparable with others taken by the same
 * ANALYSIS at the same decode rate, and it describes one particular file:
 * re-rip a track at the same path and the peaks belong to audio that is gone.
 * All four are checked before a row is believed, so a stale one re-analyses
 * itself instead of drawing the wrong picture under the right title. gen is
 * WAVE_GEN in lib/waveform.js and moves when the numbers would come out
 * different for audio that has not changed — without it, changing what is
 * measured leaves every track already analysed drawing the old shape for ever.
 *
 * A MISS IS A ROW TOO, with n = 0. A DSD file or a damaged rip cannot be
 * decoded, and without writing that down every visit to Now playing would spawn
 * ffmpeg again to fail again. The size/mtime check is what stops that being
 * permanent — repair the file and it is re-tried once.
 */
...
CREATE TABLE IF NOT EXISTS tracks (
  ...
  -- The MusicBrainz release id, when the files carry one. Picard writes it and
  -- a great many libraries are tagged with it, and it is the ONLY exact
  -- identity this app ever gets for free: with it a cover is fetched by id
  -- rather than guessed at from a name. Kept per track like the other
  -- album-level tags, for the reason above them.
...
CREATE TABLE IF NOT EXISTS cover_lookups (
  ...
  -- Which set of sources produced this answer. A miss means "none of the
  -- places we knew about had it", so it stops being true the moment a new
  -- place is added -- and a week-long cooldown would otherwise hold a
  -- library's misses against sources that were not asked. See LOOKUP_GEN in
  -- lib/covers.js. Same idea as tags_read above.
  gen      INTEGER NOT NULL DEFAULT 0
);
...
/*
 * A QUEUE FOR A ROOM THAT HOLDS NONE OF ITS OWN.
 *
 * Sonos keeps its queue on the speaker; standard AVTransport has no queue at
 * all, one URI playing and at most one to play next. So for a UPnP renderer
 * the queue lives here, and lib/queue.js's ServerQueue is the only thing that
 * reads or writes it.
 *
 * IN THE DATABASE RATHER THAN IN MEMORY, for the same reason the play history
 * is: the server restarts, and a room whose queue vanished on an update would
 * stop between tracks with nothing to say why. Position is 1-BASED, because
 * that is how Sonos numbers a queue and how every caller above already counts.
 */
...
/*
 * Where in that queue the room is.
 *
 * Its own row rather than a column on zone_queue: it is one fact about the
 * room, not a property of any track, and writing it on a track row would mean
 * deciding which one holds it.
 */
CREATE TABLE IF NOT EXISTS zone_queue_at (
...
CREATE INDEX IF NOT EXISTS idx_zone_queue     ON zone_queue(zone_id, position);
...
/*
 * A STABLE shuffle, which is the only kind a paged grid can use.
 *
 * SQLite's own RANDOM() draws again on every call, so a grid that asks for
 * albums 0-59 and then 60-119 gets two unrelated shuffles: albums appear
 * twice, others never appear at all, and scrolling the wall is nonsense. This
 * hashes the album's id together with a seed instead — the same seed always
 * produces the same order, so page two continues page one, and changing the
 * seed is what reshuffles.
 *
 * FNV-1a, because it is four lines and this is not cryptography. Declared
 * deterministic so SQLite may treat it as a constant expression per row and
 * use it in an ORDER BY without re-evaluating.
 */
...
function registerFunctions(db) {
  db.function("seeded_rank", { deterministic: true }, seededRank);
}
 ...
 *   3 — the MusicBrainz release id, which is the only exact identity a
 *       cover lookup ever gets for free
 */
const TAG_SCHEMA = 3;
...
function migrate(db) {
  ...
  add("cover_lookups", [["gen", "INTEGER NOT NULL DEFAULT 0"]]);
  /* 0 is the generation that predates the column, and no analysis ever answers
     to it — so every waveform stored before this existed re-analyses once. */
  add("waveforms", [["gen", "INTEGER NOT NULL DEFAULT 0"]]);
  add("albums", [
    ["release_date", "TEXT NOT NULL DEFAULT ''"],
    ["favourite", "INTEGER NOT NULL DEFAULT 0"],
    ["version_of", "TEXT NOT NULL DEFAULT ''"],
    ...
    ["sort_title_edit", "TEXT NOT NULL DEFAULT ''"],
    ["sort_artist_edit", "TEXT NOT NULL DEFAULT ''"],
    /* The release this album was CONFIRMED to be, by a person, in the edit
       dialog. Its own column beside the tag on the tracks for exactly the
       reason art_fetched is its own column beside art: the scan rewrites what
       the files say on every pass, so an answer this app was given has to live
       somewhere the scan does not touch. */
    ["mbid_chosen", "TEXT NOT NULL DEFAULT ''"]
  ]);
}
...
/*
 * THE RELEASE THIS ALBUM IS, in one expression, for every query that wants it.
 *
 * Two places can answer and they are not equal: `mbid_chosen` is what a person
 * confirmed in the edit dialog, and the tag on the tracks is what the files
 * happen to say. The confirmation wins, because it was made by somebody
 * looking at the record while the tag may be whatever a ripper wrote years
 * ago — and because the tag is rewritten by every scan while the confirmation
 * is not.
 *
 * One expression rather than three copies of the same COALESCE: this is the
 * same rule albumNames() exists for. Where several call sites ask the same
 * question, they ask it through one method.
 */
function albumMbid(alias = "") {
  const q = alias ? alias + "." : "";
  return `COALESCE(NULLIF(${q}mbid_chosen, ''),
                   (SELECT t.mbid FROM tracks t
                    WHERE t.album_id = ${q}id AND t.present = 1 AND t.mbid <> ''
                    LIMIT 1))`;
}
...
module.exports = {
  open, albumKey, trackKey, headAlbum, albumNames, albumMbid, seededRank,
