 ...
 *
 * rate, size and mtime are what make a stored row TRUSTWORTHY rather than
 * merely present. A waveform is only comparable with others taken at the same
 * decode rate, and it describes one particular file: re-rip a track at the same
 * path and the peaks belong to audio that is gone. All three are checked before
 * a row is believed, so a stale one re-analyses itself instead of drawing the
 * wrong picture under the right title.
 *
