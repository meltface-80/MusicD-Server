...
function createWaveforms({ db, available = true, decodeImpl = null } = {}) {
  ...
  const stmt = {
    get:   db.prepare("SELECT peaks, n, gen, rate, size, mtime FROM waveforms WHERE track_id = ?"),
    put:   db.prepare(`INSERT INTO waveforms (track_id, peaks, n, gen, rate, size, mtime, ts)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                       ON CONFLICT(track_id) DO UPDATE SET
                         peaks = excluded.peaks, n = excluded.n, gen = excluded.gen,
                         rate = excluded.rate, size = excluded.size,
  ...
  /*
   * Is a stored row still about this file, and still the shape this version
   * draws?
   *
   * Four ways it might not be, and all four are silent unless checked: the
   * ANALYSIS changed (WAVE_GEN — the numbers would come out different for the
   * same audio), the decode rate moved (so would the shape), or the file was
   * replaced at the same path — a re-rip, a re-tag, a repaired download —
   * which the size and mtime catch between them. Believing a stale row draws a
   * confident picture of audio that is no longer there, or of a measurement
   * this version does not make.
   */
  function fresh(row, track) {
    return !!row && row.gen === WF.WAVE_GEN && row.rate === DECODE_RATE &&
           row.size === (track.size || 0) && row.mtime === (track.mtime || 0);
  }

  function store(track, u8) {
    stmt.put.run(track.id, u8 ? WF.encode(u8) : "", u8 ? u8.length : 0,
                 WF.WAVE_GEN, DECODE_RATE, track.size || 0, track.mtime || 0, Date.now());
  }
