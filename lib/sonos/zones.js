...
class ZoneManager extends EventEmitter {
  ...
  async pollAll() {
    ...
    for (const id of [...this.state.keys()]) if (!live.has(id)) { this.state.delete(id); this.bump(); }
  }
  ...
  async pollGroup(g) {
    ...
    if (prev.trackUri !== next.trackUri || prev.trackNumber !== next.trackNumber) {
      ...
      next.lowFired = false;
      if (next.trackUri) this.emit("track", { zoneId: c.uid, ...next });
    } else {
    ...
    if (next.state === "playing" && !next.playedLogged && next.trackUri &&
      ...
      this.emit("played", { zoneId: c.uid, ...next });
    }
    ...
    if (next.onQueue && next.state === "playing" && next.loop === "disabled" && !next.lowFired &&
      ...
      next.lowFired = true;
      this.emit("queue-low", { zoneId: c.uid, ...next });
    }
  }
