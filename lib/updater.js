...
//
// Taken from MusicD Remote so both apps update the same way.
// Copyright (c) 2026 Lewis Menzies (Music Duck / MusicD)
// Released under the MIT License. See the LICENSE file for details.
//
// In the Docker image the new files are laid over /app inside the container
// and survive restarts of that container. A container re-created from an
// older image simply offers the update again; `docker pull` brings the image
// itself up to date.
//
...
// (which sets MUSICD_VIA_LAUNCHER=1): the running app stages the update and exits
...
// Tests point the updater at a local stand-in for GitHub.
const API = process.env.UPDATE_API || "https://api.github.com";
const web = (u) => (u.protocol === "http:" ? http : https);
const fs = require("fs");
...
// (owner-repo-sha/); the release asset built by CI wraps it in
// musicd-server/. Either way, find that single top dir.
...
// the data directory, node_modules, the staging dir, .git — or unpack the
// Android app, the Pages site and CI files a source tarball also carries).
...
function ghGetJson(apiPath, token, redirectsLeft) {
  ...
  return new Promise((resolve, reject) => {
    const u = /^https?:\/\//i.test(apiPath)
      ...
      : new URL(API + apiPath);
    const headers = {
      "User-Agent": "musicd-server-updater",
    ...
    const req = web(u).request(
      { hostname: u.hostname, port: u.port || undefined, path: u.pathname + u.search, method: "GET", headers },
      (res) => {
...
function downloadFile(url, dest, token, redirectsLeft) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (token && u.hostname === "api.github.com") headers.Authorization = "Bearer " + token;
    web(u).get({ hostname: u.hostname, port: u.port || undefined, path: u.pathname + u.search, headers }, (res) => {
...
function applyStaged(stagedDir, targetDir, opts) {
  ...
  copyOverlay(stagedDir, targetDir, [".git", ".github", "node_modules", ".update", "data", "android", "dist", "docs", "test"]);
...
function createUpdater(o) {
  ...
  async function checkNow() {
    ...
    try {
      ...
      if (r.status === 200 && r.json && r.json.tag_name) {
        ...
        throw new Error("GitHub rate-limited or forbidden (set GITHUB_TOKEN to raise the limit)");
      ...
      if (rel) {
        ...
        // Only ever forwards: a build newer than the last release (the image
        // from main between releases) is not offered a "rollback".
  ...
  async function apply() {
    ...
    try {
      ...
      let stagedName = null;
      try { stagedName = JSON.parse(fs.readFileSync(path.join(staged, "package.json"), "utf8")).name; } catch (e) {}
      if (o.packageName && stagedName !== o.packageName) {
        throw new Error("downloaded build is " + (stagedName || "something else") + ", not " + o.packageName);
      }
