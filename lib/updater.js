...
/*
 * What GitHub actually said, in words.
 *
 * "GitHub answered 403" is true and useless: the overwhelmingly common cause
 * is the unauthenticated rate limit, which is sixty requests an hour per IP
 * and is shared by every phone in the house doing its own check. Saying so,
 * and saying when it lifts, is the difference between a bug report and a
 * five-minute wait.
 */
function describeHttp(status, headers) {
  ...
  if ((status === 403 || status === 429) && remaining === "0") {
    ...
    return "GitHub is rate-limiting this address" +
      (mins ? ` — it allows sixty checks an hour and the count resets in about ${mins} minute${mins === 1 ? "" : "s"}` : "");
  }
  if (status === 404) return "GitHub has no such release (404)";
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.get({
      ...
      if (statusCode < 200 || statusCode >= 300) {
        res.resume();
        return reject(Object.assign(new Error(describeHttp(statusCode, h)),
                                    { status: statusCode }));
      }
...
/* Pinned, so a change to what the API defaults to cannot change what this
   gets back. GitHub asks for it on every documented call. */
...
function download(url, dest) {
  return get(url, {
    /*
     * Accept ANYTHING, not application/octet-stream.
     *
     * octet-stream is the Accept for a RELEASE ASSET. Ask for it on the
     * archive endpoint and GitHub answers 415 Unsupported Media Type, which is
     * what stopped every in-app update between 0.4.0 and 0.4.3. It is a header
     * a stand-in server never looks at, so nothing caught it until a real one
     * did; the fake in the suite now refuses it the same way GitHub does.
     *
     * Taking anything is the honest ask here: what comes back is a gzip from
     * codeload, and it is verified after unpacking rather than by its type.
     */
    headers: { Accept: "*/*", "X-GitHub-Api-Version": API_VERSION },
    /* One pipeline, with the size cap as a stage in it rather than a `data`
       listener alongside a pipe. Counting on the side works until it doesn't:
       attaching `data` puts the stream in flowing mode, and anything that
       arrives before the pipe is attached goes to the counter and nowhere
       else — a truncated tarball that unpacks to nothing, with no error.
       A stage in the pipeline sees every byte exactly once, and pipeline()
       cleans up all of it when any part fails. */
      ...
      const cap = new Transform({
        transform(chunk, _enc, done) {
          ...
          /* A download with no ceiling is a way to fill the disk the library
             lives on. Nothing this project publishes comes near the limit. */
          if (seen > DOWNLOAD_LIMIT) {
            return done(new Error("the download is larger than this can possibly be"));
          }
          done(null, chunk);
        }
      });
      await pipeline(res, cap, fs.createWriteStream(dest));
      if (!seen) throw new Error("the download was empty");
      return seen;
    }
  });
}
...
function createUpdater({
  ...
  function status() {
    return {
      ...
      error: state.error,
      apply: {
        phase: state.phase, error: state.phaseError, version: state.target,
        /* Only ever present after a failure, and only ever facts about this
           machine — no paths outside the install, nothing from the request. */
  ...
  /* What the process can and cannot do, gathered once when something has gone
     wrong. An update that fails does so for a reason outside the code far more
     often than inside it — no route to GitHub, no tar, nowhere to write — and
     every one of those is a different fix. Saying which, in the answer the
     phone is already polling for, is what stops the next report being "it
     fails". */
  function diagnose() {
    const out = { node: process.version, dir };
    try {
      fs.accessSync(dir, fs.constants.W_OK);
      out.writable = true;
    } catch { out.writable = false; }
    const tar = spawnSync("tar", ["--version"], { encoding: "utf8", timeout: 5000 });
    out.tar = tar.error ? "missing: " + tar.error.message
                        : (String(tar.stdout || "").split("\n")[0] || "present");
    try {
      out.free = Math.round(fs.statfsSync(dir).bavail * fs.statfsSync(dir).bsize / 1048576) + " MB";
    } catch { out.free = "unknown"; }   /* statfs is not on every platform */
    return out;
  }
  ...
  async function apply() {
    ...
    try {
      ...
      /* The phase it died in is half the diagnosis, so it is kept rather than
         overwritten by "error" — "downloading" and "unpacking" fail for
         completely different reasons. */
      const failedAt = state.phase;
      state.diagnosis = { step: failedAt, ...diagnose() };
      console.error(`[update] failed while ${failedAt}: ${e.message}`);
      console.error("[update] " + JSON.stringify(state.diagnosis));
      setPhase("error", `${e.message} (while ${failedAt})`);
