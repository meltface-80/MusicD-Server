...
/* How much of a name a PREFIX match has to carry before it means anything —
   see artistAgrees(). Four is enough to rule out "rem" inside "Remedy" and
   short enough to keep "Slowdive feat. …" agreeing with "Slowdive". */
const MIN_ARTIST_PREFIX = 4;
...
function createCovers({ db, dataDir, version, available = true, enabled = true,
  ...
  /*
   * Stylisation-folded, and a PREFIX counts: MusicBrainz credits
   * "Slowdive feat. Someone" where the tags say "Slowdive".
   *
   * IT USED TO ACCEPT CONTAINMENT ANYWHERE, which is a trap for a short name.
   * `artistKey("REM")` is "rem", and "rem" sits inside Extreme, Remedy,
   * Cremation, Bremen — and inside an anime character called Rem, which is how
   * a picker asked for R.E.M.'s "Accelerate" came back offering a cartoon. A
   * featured credit is APPENDED, never inserted, so a prefix is the whole of
   * what containment was ever for; and a prefix still has to be long enough to
   * be a name rather than a syllable.
   *
   * An exact key match cannot be rejected here — "REM" and "Rem" genuinely
   * fold to the same key — which is why the TITLE has to agree as well.
   */
  function artistAgrees(credit, artist) {
    ...
    const short = theirs.length < ours.length ? theirs : ours;
    const long = short === theirs ? ours : theirs;
    return short.length >= MIN_ARTIST_PREFIX && long.startsWith(short);
  }

  /*
   * How well a source's album name answers the one we asked about: 3 exact,
   * 2 one name inside the other, 1 most of the words, 0 not at all.
   *
   * The picker used to demand NOTHING of a title, on the grounds that a person
   * can see a wrong sleeve — which is true of one wrong sleeve and not of
   * eight. Asked for Judas Priest's "Beyond Live & Rare", a bootleg no store
   * carries, it offered the whole Judas Priest catalogue with nothing to say
   * which was which, and picking any of them would have given the bootleg
   * somebody else's cover.
   *
   * Loose rather than exact, because the names this feature exists for are the
   * bad ones: a folder called "Peter Gabriel - Scratch My Back 2010" has to
   * still reach the record called "Scratch My Back".
   */
  function titleRank(theirs, ours) {
    const a = normalise(theirs), b = normalise(ours);
    if (!a || !b) return 0;
    if (a === b) return 3;
    const at = a.split(" "), bt = b.split(" ");
    const short = at.length <= bt.length ? at : bt;
    const long = short === at ? bt : at;
    const words = new Set(long);
    const hits = short.filter(w => words.has(w)).length;
    /* A SINGLE word inside a longer name is weak evidence — "Live" is in half
       a catalogue — so it ranks with the partial matches rather than above
       them. */
    if (hits === short.length && short.length > 1) return 2;
    return hits / short.length >= 0.6 ? 1 : 0;
  }
  ...
  async function candidatesFor(albumId, title, artist) {
    ...
    /* The id in the files: exact, and free. Nothing outranks it. */
    const tagged = byTaggedId(row);
    if (tagged) {
      add({ rank: 5, url: coverUrl(tagged, "-1200"), thumb: coverUrl(tagged, "-250"),
