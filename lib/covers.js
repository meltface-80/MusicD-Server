...
/* The name-agreement rules live in lib/match.js with the rest of the matching,
   because this module is not the only one asking whether two names are the
   same record — see lib/identify.js. */
...
/* The album's MusicBrainz release id: what somebody CONFIRMED in the edit
   dialog, or failing that the tag the files carry — one expression, in
   lib/db.js, because lib/identify.js asks the same question. */
const MBID_OF = albumMbid("a");
...
/* Asking whether a picture EXISTS is a redirect, not a download, so it has no
   business taking twenty seconds — and a person is waiting on it. Failing this
   check costs nothing but the searches that would have run anyway. */
const EXISTS_TIMEOUT_MS = 6000;

/* How long a press will sit through a backoff before being told to come back.
   Longer than a normal turn, far shorter than BACKOFF_MS. */
const URGENT_PATIENCE_MS = 3000;

/* How long one rung of the size ladder gets when a person pressed the sleeve
   and is waiting for it to be saved. The sweep keeps the full timeout: nobody
   is watching a sweep. */
 ...
 *
 * A miss recorded against a NAME that has since changed is the same problem
 * wearing different clothes, which is why 3 counts: the scanner stopped
 * reading "REM - Discography" as an artist, so every album under a folder like
 * that was asked about under a name that no longer exists.
 *
 *   1 — MusicBrainz by album name, then by track names
 *   2 — the release id in the files first, and iTunes last
 *   3 — a discography folder names its artist, and a prefix is not containment
 */
const LOOKUP_GEN = 3;
...
function createCovers({ db, dataDir, version, available = true, enabled = true,
  ...
  /*
   * ONE REQUEST A SECOND, AND A QUEUE THAT LETS A PERSON THROUGH.
   *
   * It used to be a plain promise chain, which is strictly first-come — so
   * pressing Identify while a sweep of two hundred albums was running put that
   * press behind two hundred turns, four minutes of nothing. The screen looked
   * hung because it WAS: the gate written to be polite to MusicBrainz was
   * being impolite to the person holding the phone.
   *
   * So the queue has two ends. A sweep is work nobody is waiting on and it
   * joins the back; a button somebody just pressed is URGENT and goes in front
   * of every sweep turn, behind any other urgent one. The rate is untouched —
   * exactly one turn is handed out per gap, whoever holds it.
   */
  const waiting = [];
  let pumping = false;
  let lastCall = 0;
  ...
  function wait({ urgent = false } = {}) {
    /*
     * A MINUTE OF BACKOFF IS A HANG TO SOMEBODY HOLDING A PHONE.
     *
     * A 503 means back off and that is not negotiable — but making a person
     * watch a spinner for the rest of it, with nothing on screen saying why,
     * is the same silence this whole dialog exists to end. The sweep waits;
     * a press is told.
     */
    const held = holdUntil - Date.now();
    if (urgent && held > URGENT_PATIENCE_MS) {
      return Promise.reject(new Error(
        "MusicBrainz asked us to slow down. Try again in a minute."));
    }
    return new Promise((resolve) => {
      if (!urgent) waiting.push({ resolve, urgent });
      else {
        /* In front of the sweep's backlog, behind anyone else already
           waiting on a button — two people are still a queue. */
        let at = waiting.length;
        while (at > 0 && !waiting[at - 1].urgent) at--;
        waiting.splice(at, 0, { resolve, urgent });
      }
      pump();
    });
  }
  ...
  async function pump() {
    ...
    try {
      while (waiting.length) {
        ...
        lastCall = Date.now();
        /* Taken at the LAST moment, so an urgent arrival during the gap is
           already in front by the time a turn is handed out. */
        waiting.shift().resolve();
      }
    } finally { pumping = false; }
  }

  async function get(url, headers, { timeout = REQUEST_TIMEOUT_MS, redirect = "follow" } = {}) {
    ...
    try {
      ...
      /*
       * A TIMEOUT MUST NOT REACH A PERSON AS "This operation was aborted".
       * That is the DOMException's own words, it names nothing anybody can act
       * on, and it is what the edit dialog printed. Say which service and what
       * to do instead.
       */
      if (e && (e.name === "AbortError" || /abort/i.test(e.message || ""))) {
        throw new Error("That took too long to answer. Try again in a moment.");
      }
      throw e;
    } finally { clearTimeout(timer); }
  }

  async function askMusicBrainz(url, options = {}) {
    await wait(options);
   ...
   */
  async function byTitle(title, artist, options = {}) {
    ...
    const body = await askMusicBrainz(
      `${MB}/release-group/?query=${encodeURIComponent(query)}&fmt=json&limit=5`, options);
    for (const group of body["release-groups"] || []) {
      if (!group || !group.id) continue;
      if (Number(group.score) < 85) continue;
      /*
       * IT USED TO DEMAND THE TITLE EXACTLY, and that is how a record that had
       * just been identified correctly still found no cover. Fiona Apple's
       * second album is filed at MusicBrainz under its full ninety-word title;
       * a folder called "When The Pawn" is the same record and did not compare
       * equal to it, so the search returned the right release group and threw
       * it away.
       *
       * Rank 2 is "one of these names contains the other, and it is more than
       * a single word" — strong enough that "Live" does not reach "Live at
       * Leeds", loose enough that a shortened title reaches the record it is
       * short for. The artist still has to agree, and so does MusicBrainz's
       * own score.
       */
      if (titleRank(group.title || "", base) < 2) continue;
  ...
  /*
   * DOES THE ARCHIVE HOLD A PICTURE FOR THIS, asked without fetching one.
   *
   * The /front endpoints REDIRECT to the image on archive.org, which is
   * regularly slow — so following the redirect meant downloading a thumbnail
   * to learn a yes or no, and a slow one timed out and printed the browser's
   * own "This operation was aborted" into the edit dialog. The redirect ITSELF
   * is the answer: a 3xx means there is a picture, a 404 means there is not,
   * and neither costs a byte of image.
   */
  async function archiveHolds(url) {
    try {
      const res = await get(url, { Accept: "image/*" },
        { redirect: "manual", timeout: EXISTS_TIMEOUT_MS });
      return !!res.ok || (res.status >= 300 && res.status < 400);
    } catch {
      /* Slow or unreachable is not "no" — the caller falls through to a
         search, which is what this album would have had anyway. */
      return false;
    }
  }

  /*
   * THE RELEASE GROUP A RELEASE BELONGS TO.
   *
   * A release id names one PRESSING, and the archive very often holds art
   * against the record rather than against the Mexican CD of it — so an album
   * that had just been identified correctly could still find nothing. The
   * group is the same record by definition: no search, no scoring, no way to
   * reach a different one. One lookup, and only when the pressing itself came
   * up empty.
   */
  async function groupOfRelease(id, options = {}) {
    try {
      const body = await lookupMusicBrainz("release", id, "release-groups", options);
      const group = body && body["release-group"];
      return group && group.id ? String(group.id) : "";
    } catch {
      /* The pressing had no art and this did not answer; the caller searches,
         which is the same place it would have been without an id at all. */
      return "";
    }
  }
  ...
  async function download(url, options = {}) {
    ...
    const res = await get(url, { Accept: "image/*" }, options);
  ...
  async function fetchCover(hit, options = {}) {
    ...
    if (hit.url) return download(hit.url, options);
    let last = null;
    for (const size of SIZES) {
      try { return await download(coverUrl(hit, size), options); }
      catch (e) {
        last = e;
  ...
  async function candidatesFor(albumId, title, artist) {
    ...
    const list = [];
    /* `rank` is how strongly this candidate is believed to be THIS record, and
       it is what the grid is ordered by — the exact identity first, the named
       release next, and a store's guesses under them. Without an order the
       first sleeve offered is whichever source answered first, which is not a
       recommendation. */
    const add = (c) => { if (c && !list.some(x => x.url === c.url)) list.push(c); };

    /*
     * THE EXACT IDENTITY, AND THEN NOTHING ELSE.
     *
     * An album whose release is known — confirmed in this dialog, or named by
     * a tag Picard wrote — does not need searching for, and searching anyway
     * is how a record that was already right gets offered somebody else's
     * sleeve at position two. So the archive is asked about THAT release and
     * the list stops there.
     *
     * Asked rather than assumed: the archive holds no picture for plenty of
     * releases it knows by name, and an id that answers 404 must fall through
     * to the searches rather than leave the dialog empty. One request to find
     * that out, against the two a search costs.
     */
    const tagged = byTaggedId(row);
    if (tagged) {
      const said = row.chosen ? "the release you confirmed" : "the id in the files";
      const exact = [{ hit: tagged, why: said }];
      /* The pressing first, then the RECORD it is a pressing of. */
      if (!await archiveHolds(coverUrl(tagged, "-250"))) {
        exact.length = 0;
        const group = await groupOfRelease(tagged.id, { urgent: true });
        if (group) {
          exact.push({ hit: { kind: "release-group", id: group },
                       why: said + " — this record's own cover" });
        }
      }
      for (const one of exact) {
        const url = coverUrl(one.hit, "-1200"), thumb = coverUrl(one.hit, "-250");
        if (one.hit.kind === "release-group" && !await archiveHolds(thumb)) continue;
        add({ rank: 5, url, thumb, hit: one.hit, source: "Cover Art Archive", why: one.why });
      }
      if (list.length) {
        const only = list.slice(0, 9);
        offers.set(albumId, { at: Date.now(), list: only });
        return only.map((c, i) => ({ i, thumb: c.thumb, source: c.source, why: c.why }));
      }
    }

    /*
     * WHY A SOURCE GAVE NOTHING, kept for the case where none of them gave
     * anything. Trying the other source when one is down is right — they fail
     * independently — but reporting "nothing found for that name" when the
     * truth is "MusicBrainz asked us to slow down" sends somebody off to
     * correct a name that was never the problem.
     */
    let trouble = null;
    if (who && base) {
      try {
        const hit = await byTitle(name, who, { urgent: true });
        if (hit) {
          add({ rank: 4, url: coverUrl(hit, "-1200"), thumb: coverUrl(hit, "-250"),
                hit, source: "Cover Art Archive", why: "album name" });
        }
      } catch (e) {
        /* One source down is not the end of the search — but it is the whole
           of the answer if the other one is empty too. */
        trouble = trouble || e;
      }
      ...
      try {
        const res = await get(
          ...
          { Accept: "application/json" }, { timeout: EXISTS_TIMEOUT_MS });
        if (res.ok) {
          const body = await res.json();
          for (const hit of (body && body.results) || []) {
            if (!hit || !hit.artworkUrl100) continue;
            /* Looser than the automatic path, which demands the title exactly:
               a person is looking at these and can tell one wrong sleeve at a
               glance. Not looser than NOTHING, which is what it was — the
               artist and the title both have to answer, or a record no store
               carries comes back as that artist's entire catalogue. */
            if (!artistAgrees(hit.artistName || "", who)) continue;
            const rank = titleRank(hit.collectionName || "", base);
            if (!rank) continue;
            add({
              rank,
              url: String(hit.artworkUrl100).replace("100x100", "600x600"),
      ...
      } catch (e) { trouble = trouble || e; }
    }

    /* Nothing at all, and a reason for it: say the reason. An empty grid that
       blames the name is the message that sent somebody correcting a name
       which was never wrong. */
    if (!list.length && trouble) throw trouble;

    /* Best first, and a stable sort within a rank so each source keeps the
       order it answered in. Nine: three rows of three, which is as many
       sleeves as anybody compares at a glance and keeps the dialog a dialog
       rather than a search page. */
    list.sort((x, y) => y.rank - x.rank);
  ...
  async function chooseFor(albumId, index) {
    ...
    /*
     * THE SAME LADDER THE SWEEP WALKS. It used to ask for the 1200px image and
     * give up if that one request failed — so a big scan on a slow morning at
     * the archive lost the whole save, with a timeout for a message, on a
     * cover that was sitting right there at 500px. A candidate that names its
     * own image (a store's thumbnail) has no ladder; one from the archive
     * does.
     */
    /* Shorter than the sweep's patience at every rung. Nobody is watching the
       sweep; somebody IS watching this, and twenty seconds of nothing before
       the fallback even starts is the hang this dialog keeps being reported
       for. */
    const opts = { timeout: CHOSEN_TIMEOUT_MS };
    const image = chosen.hit ? await fetchCover(chosen.hit, opts)
                             : await download(chosen.url, opts);
  ...
  /* Which albums are still without one is library.withoutCover() in
     lib/library.js: it is a GRID, so it answers in the album shape every other
     grid answers in, and this module's job here is the predicate rather than
     the page. */
  ...
  /*
   * ONE GATE PER PROCESS, which is why this is handed out rather than copied.
   *
   * MusicBrainz asks for one request a second per application — per
   * APPLICATION, not per module — so lib/identify.js queues on this same
   * chain. A second module with its own timer would be two requests a second
   * from one app, which is the rate limit being broken by the code written to
   * honour it.
   */
  /*
   * One entity by its id, with whatever `inc` the caller needs — the other
   * half of the gate this module hands out. lib/info.js asks for a release
   * group's url relations through here, because the rate limit is per
   * APPLICATION and a second module with a timer of its own is two requests a
   * second from one app.
   */
  function lookupMusicBrainz(entity, id, inc = "", options = { urgent: true }) {
    const query = inc ? `?fmt=json&inc=${encodeURIComponent(inc)}` : "?fmt=json";
    return askMusicBrainz(`${MB}/${entity}/${encodeURIComponent(id)}${query}`, options);
  }

  function searchMusicBrainz(entity, query, limit = 10) {
    /* Always urgent: lib/identify.js only ever runs because somebody pressed
       Identify and is watching the dialog. */
    return askMusicBrainz(
      `${MB}/${entity}/?query=${encodeURIComponent(query)}&fmt=json&limit=${limit}`,
      { urgent: true });
  }

  return { sweep, findFor, status, setEnabled, pending,
           searchMusicBrainz, lookupMusicBrainz, groupOfRelease,
