...
function createCovers({ db, dataDir, version, available = true, enabled = true,
  ...
  async function candidatesFor(albumId, title, artist) {
    ...
    if (tagged) {
      const url = coverUrl(tagged, "-1200"), thumb = coverUrl(tagged, "-250");
      let held = false;
      try {
        const res = await get(thumb, { Accept: "image/*" });
        held = !!res.ok;
      } catch { /* unreachable is not "no": fall through and search as usual */ }
      if (held) {
        add({ rank: 5, url, thumb, source: "Cover Art Archive",
              why: row.chosen ? "the release you confirmed" : "the id in the files" });
