...
const { albumKey, trackKey } = require("./db");
...
/* The album artist, decided once for the whole folder. Tags disagree far more
   often than they agree, so the rule is: use the album-artist tag if the
   folder is unanimous about it, otherwise the track artist if THAT is
   unanimous, otherwise call it a compilation. */
function deriveAlbumArtist(tracks) {
  const albumArtists = new Set(tracks.map(t => t.albumartist).filter(Boolean));
  if (albumArtists.size === 1) return [...albumArtists][0];
  if (albumArtists.size > 1) return "Various Artists";
  const artists = new Set(tracks.map(t => t.artist).filter(Boolean));
  if (artists.size === 1) return [...artists][0];
  if (artists.size > 1) return "Various Artists";
  return "";
}

function deriveAlbumTitle(tracks, dir) {
  const titles = new Set(tracks.map(t => t.album).filter(Boolean));
  if (titles.size === 1) return [...titles][0];
  /* Disagreeing or absent album tags: the folder name is the honest answer,
     with a leading "01 - " style index stripped if one is there. */
  return path.basename(dir).replace(/^\d{1,4}\s*[-–.]\s*/, "").trim() || path.basename(dir);
}
  ...
  for (const folder of folders) {
    ...
    for (const name of folder.files.sort((a, b) => a.localeCompare(b, undefined, { numeric: true }))) {
      ...
      if (prev && prev.mtime === mtime && prev.size === st.size) {
        /* Untouched since the last scan. The stored row carries the album-level
           tags too, so the folder's artist, year and genre can still be derived
           from every file in it — not just from the one that changed. */
        rows.push({
    ...
    const artist = deriveAlbumArtist(rows);
    const year   = commonest(rows.map(r => r.year).filter(Boolean)) || null;
