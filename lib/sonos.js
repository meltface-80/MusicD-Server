...
/*
 * WHAT A SONOS WILL PLAY, and the one player whose answer is KNOWN rather than
 * asked — the same trade its table of control URLs makes.
 *
 * It lived in lib/scanner.js until 0.4.54, which was the wrong file twice
 * over: the scan never used it, and a fact about one manufacturer's speakers
 * has no business in the thing that reads folders. It is here because THE TEST
 * for which file something belongs in is whether a device that never heard of
 * Sonos would still need it, and none of them needs this.
 *
 * BY EXTENSION RATHER THAN MIME, on purpose. Sonos's own ConnectionManager
 * does implement GetProtocolInfo and answers with hundreds of entries,
 * including formats that do not work from a third-party HTTP server — and
 * MIME cannot express the distinction that matters here anyway: Opus maps to
 * audio/ogg, which Sonos plays, and Opus inside it is not.
 */
const SONOS_PLAYABLE = new Set([
  ".flac", ".mp3", ".m4a", ".aac", ".alac", ".wav", ".wave",
  ".aif", ".aiff", ".aifc", ".ogg", ".oga"
]);
function playableBySonos(ext) { return SONOS_PLAYABLE.has(String(ext || "").toLowerCase()); }
...
class Player {
  ...
  /* Sonos holds its own queue — the tracks live on the speaker, it moves
     between them itself, and reading it back is a Browse. See queueFor() in
     lib/playback.js, which is what asks. */
  get holdsQueue() { return true; }

  /*
   * WILL THIS SPEAKER TAKE THIS FILE? Sonos is the one player whose answer is
   * KNOWN rather than asked, which is the same trade its control-URL table
   * makes. Its ConnectionManager does implement GetProtocolInfo, and it
   * answers with hundreds of entries including formats that do not actually
   * work from a third-party HTTP server — so the curated list wins, and it is
   * kept by EXTENSION rather than MIME on purpose: Opus maps to audio/ogg,
   * which Sonos does play, and Opus inside it is not.
   */
  plays(file) { return playableBySonos((file && file.ext) || ""); }

  /* For the sentence a person reads when nothing on an album will play. Stated
     in containers rather than MIME types, because that is what somebody looking
     at their own files has to go on. */
  get playsWhat() {
    return "FLAC, ALAC, WAV, AIFF, MP3, AAC and Ogg Vorbis — but not DSD, WMA or Opus";
  }
...
class Household {
  constructor(opts = {}) {
    ...
    /* Injectable ONLY so the no-seeds path can be driven by a test. It is the
       path a real install with no SONOS_HOSTS uses for every room it has, and
       it was the one path nothing covered — every test seeds an address, so
       _topologyFromAny() answered on the first try and multicast was never
       reached. That is how a broken discovery call passed 544 tests. */
    this.discover = opts.discover || findZonePlayers;
  }
  ...
  async _doRefresh() {
    ...
    if (!zones.length) {
      /* findZonePlayers(), NOT the generic ssdpSearch() this file imports.
         That one wants a search target and answers with {ip, location} pairs;
         this one asks for ZonePlayers and answers with the plain addresses
         _topologyFromAny() expects. Calling the wrong one is not a type error
         in JavaScript — it is a search for ST: undefined that nothing answers,
         and every room silently disappearing. It shipped in 0.4.43. */
 ...
 */
module.exports = {
  Household, Player, parseZoneGroupState, findZonePlayers, playableBySonos,
  SONOS_PORT
};
