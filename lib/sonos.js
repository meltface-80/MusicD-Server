...
class Player {
  ...
  /*
   * A RUN of queue positions, gone in one call.
   *
   * Sonos numbers the queue from 1 and renumbers what is left the moment
   * something is taken out — so a caller removing several runs has to work
   * from the END backwards, or the second range names tracks that have already
   * shifted under it. See removeFromQueue() in lib/playback.js, which is the
   * only thing that calls this.
   */
  removeTrackRange(start, count) {
    return this.avt("RemoveTrackRangeFromQueue", {
