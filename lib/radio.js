...
/*
 * PER ROOM AS OF 0.4.44, and the old keys are how that happened without a
 * migration.
 *
 * These two were one switch for the whole house, applied by the poll loop to
 * every coordinator it found. They are now `radio.enabled.<zoneId>` — and when
 * a room has no answer of its own, the OLD global key is read instead. An
 * install that had the radio on therefore has it on in every room, which is
 * precisely what it had; the first per-room write starts overriding, and
 * nothing has to be upgraded, copied or guessed at.
 *
 * The same trick lib/settings.js uses for ROWS_OFF_KEY: absence is a default,
 * not a blank.
 */
const ENABLED_KEY = "radio.enabled";
const GENRE_KEY = "radio.matchGenre";

/* Zone ids come off the wire. A settings key is built from one, so the shape
   is checked rather than trusted: what a room is called is not this module's
   business, but "not a key that could collide with another setting" is. */
const ZONE_ID = /^[A-Za-z0-9_.:-]{1,64}$/;

function zoneKey(base, zoneId) {
...
function createRadio({ db, settings }) {
  ...
  /*
   * READ THROUGH TO THE DATABASE rather than cached in here.
   *
   * The old single switch was held in a variable because there was one of it.
   * A cache per room would be a Map to keep in step with every write, and this
   * is asked once per TRACK CHANGE per room — a primary-key lookup on a table
   * with a handful of rows in it. Correctness is worth more than that.
   */
  function raw(base, zoneId) {
    const own = settings.get(zoneKey(base, zoneId));
    /* No answer for this room: the pre-0.4.44 global, which is the answer this
       install already had for every room. */
    return own === null ? settings.get(base) : own;
  }
  ...
  function enabledIn(zoneId) {
    return raw(ENABLED_KEY, zoneId) === "1";
  }

  /* On by default ONCE THE RADIO IS ON in that room: an evening of one genre is
     what most people mean by radio, and the setting is right there to turn
     off. It has no effect at all while the radio is off. */
  function matchGenreIn(zoneId) {
    return raw(GENRE_KEY, zoneId) !== "0";
  }

  function status(zoneId) {
    return { enabled: enabledIn(zoneId), matchGenre: matchGenreIn(zoneId) };
  }

  function setEnabled(zoneId, on) {
    settings.set(zoneKey(ENABLED_KEY, zoneId), on ? "1" : "0");
    return status(zoneId);
  }

  function setMatchGenre(zoneId, on) {
    settings.set(zoneKey(GENRE_KEY, zoneId), on ? "1" : "0");
    return status(zoneId);
  }
  ...
  /* Whether to bother asking THIS room's speaker at all. Keeps the extra SOAP
     call out of the poll loop entirely for a room nobody turned the radio on
     in — which, now that it is per room, is most of them. */
  function wanted(zoneId) { return enabledIn(zoneId); }
