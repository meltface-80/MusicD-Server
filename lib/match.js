...
/*
 * DOES THIS CREDIT NAME THE ARTIST WE ASKED ABOUT?
 *
 * Stylisation-folded, and a PREFIX counts: MusicBrainz credits "Slowdive feat.
 * Someone" where the tags say "Slowdive".
 *
 * It used to accept containment ANYWHERE, which is a trap for a short name.
 * artistKey("REM") is "rem", and "rem" sits inside Remedy, Extreme, Cremation
 * — and inside an anime character called Rem, which is how a cover picker
 * asked for R.E.M.'s "Accelerate" came back offering a cartoon. A featured
 * credit is APPENDED, never inserted, so a prefix is the whole of what
 * containment was ever for; and a prefix still has to be long enough to be a
 * name rather than a syllable.
 *
 * An exact key match cannot be rejected here — "REM" and "Rem" genuinely fold
 * to the same key — which is why every caller checks the TITLE as well.
 */
...
/*
 * How well a source's album name answers the one we asked about: 3 exact, 2
 * one name inside the other, 1 most of the words, 0 not at all.
 *
 * Loose rather than exact, because the names this exists for are the BAD ones:
 * a folder called "Peter Gabriel - Scratch My Back 2010" has to still reach the
 * record called "Scratch My Back". Zero relevance is still zero, though — a
 * picker that demands nothing of a title answers a bootleg with the artist's
 * whole catalogue.
 */
function titleRank(theirs, ours) {
  ...
  /* A SINGLE word inside a longer name is weak evidence — "Live" is in half a
     catalogue — so it ranks with the partial matches rather than above them. */
...
module.exports = {
  ...
  artistAgrees, titleRank, MIN_ARTIST_PREFIX,
