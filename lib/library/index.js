...
class Library {
  constructor(db, { musicRoot = "/music", log = () => {} } = {}) {
    ...
    this.q = {
      ...
      merges: this.raw.prepare("SELECT source, target FROM label_merges")
    };
  }

  labelFolderDepth() { return Number(this.db.setting("labelFolderDepth", 0)) || 0; }

  /* Rebuild the in-memory copy from the database. */
  reload() {
    const rows = this.raw.prepare("SELECT * FROM albums").all();
    const depth = this.labelFolderDepth();
    const merges = new Map(this.q.merges.all().map(r => [r.source, r.target]));
    const albums = rows.map(r => {
      ...
      let label = r.label || null;
      if (!label && depth > 0 && r.dir) {
        const rel = path.relative(this.musicRoot, r.dir).split(path.sep).filter(Boolean);
        if (rel.length >= depth) label = rel[depth - 1];
      }
      if (label) {
        const target = merges.get(N.fold(label));
  ...
  searchLabels(query) {
    ...
    if (!q) return [];
    return this.labels()
      .filter(l => N.fold(l.title).includes(q))
      .map(l => ({ display: l.title, albumCount: l.albumCount, logo_url: null }))
      .sort((a, b) => (N.fold(a.display).startsWith(q) ? 0 : 1) - (N.fold(b.display).startsWith(q) ? 0 : 1) || b.albumCount - a.albumCount)
      .slice(0, 10);
  }
  ...
  labelGroups() {
    const m = new Map();
    for (const al of this.albums) {
      if (!al.label) continue;
      const k = N.fold(al.label);
      if (!k) continue;
      if (!m.has(k)) m.set(k, { key: k, display: al.label, albums: [] });
      m.get(k).albums.push(al);
    }
    return m;
  }

  labels() {
    const merges = this.q.merges.all();
    const byTarget = new Map();
    for (const r of merges) {
      const t = N.fold(r.target);
      if (!byTarget.has(t)) byTarget.set(t, []);
      byTarget.get(t).push({ key: r.source, display: r.source });
    }
    return [...this.labelGroups().values()].map(e => ({
      key: e.key,
      title: e.display,
      subtitle: `${e.albums.length} album${e.albums.length === 1 ? "" : "s"}`,
      albumCount: e.albums.length,
      image_key: e.albums[0].image_key,
      logo_url: null,
      mergedFrom: byTarget.get(e.key) || []
    })).sort((a, b) => a.title.localeCompare(b.title, undefined, { sensitivity: "base" }));
  }

  labelAlbums(name) {
    const e = this.labelGroups().get(N.fold(name));
    return e ? e.albums.slice() : [];
  }
  ...
  facetDefs() {
    return [
      ...
      { id: "label", label: "Record label", values: al => al.label ? [al.label] : [] },
  ...
  facets() {
    ...
    return {
      ...
      facets,
      coverage: {
        decade: countWith("decade"), genre: countWith("genre"), label: countWith("label"),
  ...
  filtered(f) {
    ...
    if (f.type === "label") return this.albums.filter(al => al.label && N.fold(al.label) === v);
    if (f.type === "decade") {
