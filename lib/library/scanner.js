...
class Scanner {
  ...
  async scan({ force = false } = {}) {
    ...
    // Files that weren't found are removed — unless the folder they were in is
    // there but empty. That is a drive that isn't mounted (yet), not a
    // library that was deleted: its tracks are kept, and checked again next
    // scan, rather than thrown away and read from scratch when it's back.
    const topOf = p => path.relative(this.root, p).split(path.sep)[0];
    const seenTops = new Set([...seen].map(topOf));
    ...
    this.db.transaction(() => {
      for (const [p, r] of known) {
        ...
        const top = topOf(p);
        const topDir = path.join(this.root, top);
        const unmounted = (!seen.size && isEmptyDir(this.root)) ||
          (!seenTops.has(top) && top !== path.basename(p) && isEmptyDir(topDir));
        if (unmounted) { const d = seen.size || !isEmptyDir(this.root) ? topDir : this.root; offline.set(d, (offline.get(d) || 0) + 1); continue; }
        this.q.deleteTrack.run(r.id); touchedAlbums.add(r.album_id); removed++;
      }
    })();
    for (const [d, n] of offline) {
      this.log(`[scan] ${d} is empty — kept its ${n} tracks as they were (a drive not mounted?)`);
    }
    this.db.transaction(() => {
    ...
    const result = {
      status: (added || changed || removed || relocated) ? "updated" : "unchanged",
      added, changed, removed, relocated, kept_offline: [...offline.values()].reduce((a, b) => a + b, 0), albums, tracks,
