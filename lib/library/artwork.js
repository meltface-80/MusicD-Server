...
// Pasted addresses can be anywhere, and some image hosts turn away a request
// that doesn't look like a browser's.
async function fetchImage(url) {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: { "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36", "Accept": "image/*,*/*;q=0.8" }
    });
...
class Artwork {
  ...
  async source(key) {
    ...
    if (!al) return null;
    if (al.customArt) {
      const buf = this.library.editedArt(al.id);
      if (buf) return { buf, id: `${al.id}-e` };
    }
    if (al.artPath) {
  ...
  /* Does the album have a cover of its own — a found one, a file in its
   * folder, or one embedded in its tracks? (Not the drawn placeholder.) */
  async hasOwnArt(al) {
    if (!al) return false;
    if (al.customArt) return true;
    if (al.artPath && fs.existsSync(al.artPath)) return true;
    for (const t of this.library.tracks(al.id).slice(0, 3)) {
      try { if (await embeddedPicture(t.path)) return true; } catch (e) { /* next */ }
    }
    return false;
  }

  /* A cover fetched from the web or pasted as a URL, checked to really be a
   * picture and brought to a sensible size before it is stored. */
  static async download(url) {
    if (!/^https?:\/\//i.test(url)) throw Object.assign(new Error("That isn't a web address"), { status: 400 });
    let buf;
    try { buf = await fetchImage(url); } catch (e) { throw Object.assign(new Error("Couldn't download that image (" + e.message + ")"), { status: 502 }); }
    let meta;
    try { meta = await sharp(buf, { failOn: "none" }).metadata(); } catch (e) { meta = null; }
    if (!meta || !meta.width) throw Object.assign(new Error("That address isn't an image"), { status: 422 });
  ...
  /* A cover changed (found, removed, or edits undone): the sizes rendered for
   * the album's earlier covers go, so any old address still held somewhere —
   * a tile drawn before the edit, a Sonos queue from before it — is drawn
   * again from the current cover instead of serving the old picture. */
  forget(albumId, keepKey) {
    const prefix = `al-${Number(albumId)}-`;
    const keep = keepKey ? keepKey.replace(/[^A-Za-z0-9_-]/g, "") + "@" : null;
    let n = 0;
    try {
      for (const f of fs.readdirSync(this.dir)) {
        if (!f.startsWith(prefix) || (keep && f.startsWith(keep))) continue;
        try { fs.unlinkSync(path.join(this.dir, f)); n++; } catch (e) { /* in use: next time */ }
      }
