...
module.exports = function mountPlayback(app, ctx) {
  ...
  /*
   * Phone playback is the Android app's own: a phone zone exists only for
   * the phone it is. Every other device — the iPhone home-screen app, any
   * browser — neither sees it nor can send anything to it.
   */
  const myDevice = req => (ctx.auth && ctx.auth.deviceOf(req)) || null;
  const mine = (z, req) => !z.is_phone || (myDevice(req) || {}).id === z.device_id;
  const PHONE_FIELDS = ["zone", "zone_or_output_id", "output_id", "zone_id", "from", "to"];
  app.use("/api", (req, res, next) => {
    if (req.path.startsWith("/phone/") || req.path.startsWith("/auth/")) return next();
    const dev = myDevice(req);
    const own = dev ? zones.phones.constructor.uidFor(dev.id) : null;
    const b = req.body || {};
    const ids = PHONE_FIELDS.flatMap(k => [req.query[k], b[k]]).concat(Array.isArray(b.output_ids) ? b.output_ids : []);
    if (ids.some(id => zones.phones.isPhoneId(id) && id !== own)) {
      return res.status(403).json({ error: "Phone playback is only for the phone itself" });
    }
    next();
  });

  // Only ever seen by the phone itself, as "This phone".
  function nameFor(z, req) {
    return z.is_phone ? "This phone" : z.display_name;
  }
  function named(z, req) {
    if (!z.is_phone) return z;
    const name = nameFor(z, req);
    return Object.assign({}, z, {
      display_name: name,
      outputs: z.outputs.map(o => Object.assign({}, o, { display_name: name }))
    });
  }
