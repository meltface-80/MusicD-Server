...
module.exports = function mountPlayback(app, ctx) {
  ...
  // A phone sees itself as "This phone"; everyone else sees its name.
  function nameFor(z, req) {
    if (!z.is_phone || !req) return z.display_name;
    const dev = ctx.auth && ctx.auth.deviceOf(req);
    return dev && dev.id === z.device_id ? "This phone" : z.display_name;
  }
  ...
  app.get("/api/zones", (req, res) => {
    res.json({
      zones: zones.zones().map(z => named(z, req)).map(z => ({
  ...
  app.get("/api/shortcut/zones", (req, res) => {
    ...
    res.json({
      ...
      last_zone: last && zones.zone(last) ? zones.zone(last).zone_id : ((list.find(z => z.state === "playing") || list[0] || {}).zone_id || null)
    });
  });
