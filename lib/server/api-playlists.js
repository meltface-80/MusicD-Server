...
module.exports = function mountPlaylists(app, ctx) {
  ...
  // -------------------------------------------------------------- updates
  //
  // One-tap updates, the same way MusicD Remote does them: the newest GitHub
  // release (published by CI for every version merged to main) is downloaded,
  // unpacked, and swapped in by launcher.js while the server is stopped.

  const [owner, repo] = (process.env.UPDATE_REPO || "meltface-80/MusicD-Server").split("/");
  const updater = Updater.createUpdater({
    owner, repo, currentVersion: ctx.version, packageName: "musicd-server",
    dir: path.join(__dirname, "..", ".."),
    viaLauncher: process.env.MUSICD_VIA_LAUNCHER === "1",
    token: process.env.GITHUB_TOKEN || null,
    debug: !!process.env.DEBUG
  });
  if (process.env.UPDATE_CHECK !== "false") {
    setTimeout(() => updater.startAuto(12 * 3600 * 1000), 30000).unref();
  }
  const updStatus = () => Object.assign(updater.getStatus(), { is_docker: process.env.DOCKER === "1" });

  app.get("/api/update/status", (req, res) => res.json(updStatus()));
  app.post("/api/update/check", async (req, res) => { await updater.checkNow(); res.json(updStatus()); });
  app.post("/api/update/apply", async (req, res) => {
    const st = await updater.apply();
    if (st.apply.phase === "error") return res.status(409).json(Object.assign({ error: st.apply.error }, { status: updStatus() }));
    res.json({ ok: true, status: updStatus() });
  });
};
