...
module.exports = function mountLibrary(app, ctx) {
  ...
  app.get("/api/status", (req, res) => {
    // Away from home the rooms aren't this device's to see (api-playback.js).
    const away = !!(ctx.auth && ctx.auth.isAway(req));
    res.json({
      away,
      paired: true,
      core_id: "musicd-server",
      core_name: "MusicD Server",
      zone_count: away ? 0 : zones.zones().length,
  ...
  // "downloads" is the Android app's row (albums on that phone); other devices
  // leave it out. It starts off, at the top, and the app turns it on the
  // first time an album is downloaded.
  const HOME_ROWS = ["downloads", "unplayed", "history", "picks", "random", "library", "genres"];
  const defaultRow = id => ({ id, on: id !== "downloads" });
  function homeRows() {
    ...
    for (const id of HOME_ROWS) {
      if (seen.has(id)) continue;
      if (id === "downloads") out.unshift(defaultRow(id)); else out.push(defaultRow(id));
    }
    return out;
  }
