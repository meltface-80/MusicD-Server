...
const { fnv1a } = require("../library/index");
...
module.exports = function mountLibrary(app, ctx) {
  ...
  function parseFilter(src) {
    ...
    if (!["genre", "tag", "label", "decade"].includes(type)) return null;
  ...
  function trackRows(al) {
    ...
    return tracks.map(t => {
      const n = t.track_no ? (discs.size > 1 ? `${t.disc_no || 1}.${t.track_no}` : String(t.track_no)) : "";
      ...
      return {
        title: (n ? n + ". " : "") + t.title,
        subtitle: [credit, fmtLen(t.duration)].filter(Boolean).join(" · "),
  ...
  app.get("/api/album", (req, res) => {
    ...
    res.json({
      album: Object.assign(library.json(al), { year: al.year, label: al.label, genres: al.genres }),
      tracks,
      actions: [
  ...
  app.get("/api/search", (req, res) => {
    ...
    res.json({
      ...
      labels: library.searchLabels(q), artists: library.searchArtists(q)
    });
  });
  ...
  app.get("/api/home/label-of-the-week", (req, res) => {
    ...
    const groups = [...library.labelGroups().values()].filter(e => e.albums.length >= 6).sort((a, b) => a.key.localeCompare(b.key));
    if (!groups.length) return res.json({ label: null, albums: [] });
    const e = groups[fnv1a(isoWeekKey()) % groups.length];
    res.json({ label: e.display, albums: e.albums.slice(0, 24).map(a => library.json(a)) });
  });
  ...
  const labelsEnabled = () => db.setting("labelsEnabled", true);

  app.get("/api/filters/labels", (req, res) => {
    if (!labelsEnabled()) return res.json({ labels: [], scanning: false, progress: 0 });
    res.json({ labels: library.labels(), scanning: scanner.state.running && !library.count, progress: scanner.state.progress, count: library.count });
  });
  app.get("/api/label-albums", (req, res) => {
    ...
    let albums = library.labelAlbums(name);
    if (order === "random") albums.sort(() => Math.random() - 0.5);
    else albums.sort((a, b) => a.sortTitle.localeCompare(b.sortTitle));
    res.json({ albums: albums.map(a => library.json(a)), total: albums.length, label: name, order, groupKey: N.fold(name), logo_url: null });
  });
  app.get("/api/labels-scan-status", (req, res) => res.json({
    scanning: scanner.state.running, progress: scanner.state.progress, count: library.labels().length, builtAt: library.builtAt
  }));
  app.get("/api/labels-scan-log", (req, res) => res.json({ lines: [
    "Labels come from your files' own tags (LABEL / ORGANIZATION / PUBLISHER).",
    labelFolderDepth() ? `Albums with no label tag use folder level ${labelFolderDepth()} under ${ctx.config.musicDir}.` : "Set Label from folder depth in Settings to fill gaps from your folder layout."
  ] }));
  const labelFolderDepth = () => Number(db.setting("labelFolderDepth", 0)) || 0;
  app.post("/api/labels/rescan", wrap(async (req, res) => { library.reload(); res.json({ ok: true, count: library.labels().length }); }));
  app.post("/api/labels/rescan-force", wrap(async (req, res) => { library.reload(); res.json({ ok: true, count: library.labels().length }); }));
  app.post("/api/labels/merge", (req, res) => {
    const { source, target } = req.body || {};
    if (!source || !target) return res.status(400).json({ error: "source and target required" });
    db.raw.prepare("INSERT OR REPLACE INTO label_merges(source, target) VALUES(?, ?)").run(N.fold(source), String(target));
    library.reload();
    res.json({ ok: true });
  });
  app.delete("/api/labels/merge/:sourceKey", (req, res) => {
    db.raw.prepare("DELETE FROM label_merges WHERE source = ?").run(N.fold(req.params.sourceKey));
    library.reload();
    res.json({ ok: true });
  });
  app.get("/api/labels/logo-candidates", (req, res) => res.json({ candidates: [] }));
  app.post("/api/labels/logo", (req, res) => res.status(501).json({ error: "Label logos aren't in MusicD Server yet — labels show as text tiles" }));

  app.get("/api/settings/labels", (req, res) => res.json({ enabled: labelsEnabled(), count: library.labels().length, scanning: false }));
  app.post("/api/settings/labels", (req, res) => {
    if ((req.body || {}).enabled === undefined) return res.status(400).json({ error: "enabled required" });
    db.setSetting("labelsEnabled", !!req.body.enabled);
    res.json({ ok: true, enabled: !!req.body.enabled });
  });
  app.get("/api/settings/label-folder-depth", (req, res) => res.json({ depth: labelFolderDepth() }));
  app.post("/api/settings/label-folder-depth", (req, res) => {
    const d = parseInt((req.body || {}).depth, 10);
    if (!Number.isFinite(d) || d < 0 || d > 8) return res.status(400).json({ error: "depth must be 0-8" });
    db.setSetting("labelFolderDepth", d);
    library.reload();
    res.json({ ok: true, depth: d });
  });

  for (const [route, key, field] of [["discogs-token", "discogsToken", "token"], ["fanart-key", "fanartKey", "key"]]) {
    app.get(`/api/settings/${route}`, (req, res) => res.json({ set: !!db.setting(key, ""), configured: !!db.setting(key, "") }));
  ...
  app.get("/api/image/:key", wrap(async (req, res) => {
    ...
    const got = await artwork.get(req.params.key, size);
  ...
  app.get("/api/album/extras", wrap(async (req, res) => {
    ...
    // The file's own label tag outranks anything scraped.
    if (al && al.label) {
      if (!bios) bios = { album: null, artist: null };
      if (!bios.album) bios.album = {};
      bios.album.label = al.label;
    }
  ...
  app.post("/api/settings/share-links", (req, res) => {
    ...
    if (!touched) return res.status(400).json({ error: "services and/or reviews array required" });
    res.json({ ok: true, services: ctx.shareServices(), reviews: ctx.shareReviews() });
  });
  ...
  app.get("/api/display/content", wrap(async (req, res) => {
    ...
    const moreLabel = al && al.label ? library.labelAlbums(al.label).filter(a => a.id !== al.id).slice(0, 12) : [];
    res.json({
      ...
      moreAlbums: {
        ...
        label: moreLabel.length >= 3 ? { name: al.label, albums: moreLabel.map(a => library.json(a)) } : null
      }
    });
  }));
  ...
  const HOME_ROWS = ["unplayed", "history", "picks", "random", "library", "lotw", "genres"];
  function homeRows() {
  ...
  app.get("/api/settings/home-rows", (req, res) => res.json({
    rows: homeRows().map(r => Object.assign({}, r, {
      unavailable: r.id === "picks" && !features.smartSettings().enabled ? "Smart Picks is off in Settings"
        : r.id === "lotw" && !labelsEnabled() ? "Labels is off in Settings" : null
    }))
  }));
