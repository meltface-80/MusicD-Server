...
function mount(app, ctx) {
  ...
  /*
   * What the phone keeps by itself (automatic downloads): today's Smart
   * Picks, the Album of the day, and the most recently added albums. The
   * phone asks now and then, downloads what's new and lets go of what has
   * dropped off.  ?picks=1&aotd=1&recent=<n, up to 50>
   */
  app.get("/api/download/auto", (req, res) => {
    const out = new Map();
    const add = (al, source) => {
      if (!al) return;
      const e = out.get(al.id) || { id: al.id, title: al.title, artist: al.artist, sources: [] };
      if (!e.sources.includes(source)) e.sources.push(source);
      out.set(al.id, e);
    };
    const on = v => v === "1" || v === "true";
    if (on(req.query.picks) && ctx.features && ctx.features.smartSettings().enabled) {
      for (const p of ctx.features.smartPicksJson().picks) add(library.album(p.offset), "picks");
    }
    if (on(req.query.aotd)) add(library.albumOfTheDay(), "aotd");
    const recent = Math.max(0, Math.min(50, parseInt(req.query.recent, 10) || 0));
    if (recent) for (const al of library.view({ sort: "added", dir: "desc" }).slice(0, recent)) add(al, "recent");
    res.json({ albums: [...out.values()] });
  });
