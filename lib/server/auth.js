...
// (100.64.0.0/10, and fd7a:115c:a1e0::/48 inside the private IPv6 range) are
// not: a request over Tailscale is away from home.
function isLocal(ip) {
  let a = String(ip || "");
  if (a.startsWith("::ffff:")) a = a.slice(7);
  if (a === "::1" || a.startsWith("127.")) return true;
  if (/^fd7a:115c:a1e0:/i.test(a)) return false;
...
const bare = a => { a = String(a || "").trim(); return a.startsWith("::ffff:") ? a.slice(7) : a; };
const loopback = a => a === "::1" || a.startsWith("127.");

// Who is asking. A proxy on this machine (`tailscale serve`, say) speaks for
// the device behind it, so its X-Forwarded-For is believed — only from there.
function clientIp(req) {
  const a = bare(req.socket && req.socket.remoteAddress);
  if (loopback(a)) {
    const fwd = String(req.headers["x-forwarded-for"] || "").split(",").map(bare).filter(Boolean);
    if (fwd.length) return fwd[fwd.length - 1];
  }
  return a;
}

// Away from home: anything not on the home network, Tailscale included.
const isAway = req => !isLocal(clientIp(req));
...
function createAuth(ctx) {
  ...
  return { gate, mount, signUrl, deviceOf, isLocal, isAway, clientIp, account };
}

module.exports = { createAuth, isLocal, isAway, clientIp, deviceNameFromUA, COOKIE };
