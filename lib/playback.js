...
const { SonosQueue, NoQueue } = require("./queue");
...
class Playback {
  ...
  queueFor(player) {
    /* ASKED, not inferred from which methods happen to exist. A player that
       holds no queue gets one that refuses in a sentence, because the
       alternative is a device being sent an action it never claimed to
       implement and a UPnP fault reaching a screen. */
    return player.holdsQueue ? new SonosQueue(player) : new NoQueue(player);
  }
  ...
  async queue(zoneUuid, limit = 200) {
    ...
    const [pos, browsed] = await Promise.all([
      coord.positionInfo(), this.queueFor(coord).list(limit)
    ]);
  ...
  async topUp(coord, track, pos) {
    ...
    /* positionInfo's track number is 1-based and this array is not, so cutting
       at it drops everything up to AND INCLUDING the one playing. */
    const remaining = albums.slice(Math.max(0, pos.track || 1))
      .map(albumId => ({ albumId }));
    ...
    for (const [uuid, coord] of coordinators) {
      ...
      if (!seen || seen.trackId !== track.id) {
        ...
        /* ON A NEW TRACK ONLY, which is what keeps the radio cheap: reading the
           ...
           the next room. */
        this.topUp(coord, track, pos).catch(e => {
