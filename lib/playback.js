...
class Playback {
  ...
  /* What lib/didl.js needs to describe one track, in one place — both the
     single-item document and the batched one are built from this, so they
     cannot start describing the same track differently. */
  didlOptionsFor(track, album) {
    return {
  ...
  metadataFor(track, album) {
    return didl.trackItem(track, this.didlOptionsFor(track, album));
  }
  ...
  async enqueue(zoneUuid, tracks, { replace = true, startTrackId = null, autoplay = true } = {}) {
    ...
    await this.addAll(player, playable, albums);
   ...
   */
  async addAll(player, playable, albums) {
    const entry = (t) => ({
      track: t, uri: this.trackUri(t),
      ...this.didlOptionsFor(t, albums.get(t.album_id) || {})
    });
    try {
      for (let at = 0; at < playable.length; at += QUEUE_BATCH) {
        const batch = playable.slice(at, at + QUEUE_BATCH).map(entry);
        await player.addManyToQueue(batch.map(e => e.uri), didl.trackItems(batch));
      }
      return;
    } catch (e) {
      /* Not silent: a player that refuses the batch is worth knowing about,
         and the queue is still filled below. */
      console.warn("[playback] batched enqueue refused (" + e.message +
  ...
  /*
   * TAKE THESE POSITIONS OUT OF THE QUEUE.
   *
   * Sonos numbers the queue from 1 and RENUMBERS what is left the instant
   * something is removed, so the order of these calls is the whole of the
   * correctness: contiguous positions are collapsed into one range, and the
   * ranges are applied from the END backwards so that every index is still
   * the index it was when the caller read it.
   */
  async removeFromQueue(zoneUuid, indexes) {
    await this.household.refresh();
    const player = this.household.coordinatorFor(zoneUuid);
    if (!player) throw new Error("That room is not on the network right now.");

    const wanted = [...new Set(indexes.map(Number).filter(i => Number.isInteger(i) && i > 0))]
      .sort((a, b) => a - b);
    if (!wanted.length) return { removed: 0 };

    /* Runs of consecutive positions, built low to high and then walked back
       down — one call per run instead of one per track. */
    const runs = [];
    ...
    for (let at = runs.length - 1; at >= 0; at--) {
      await player.removeTrackRange(runs[at].start, runs[at].count);
    }
    return { removed: wanted.length };
  }

  /* Everything, in one call. */
  async clearQueue(zoneUuid) {
    await this.household.refresh();
    const player = this.household.coordinatorFor(zoneUuid);
    if (!player) throw new Error("That room is not on the network right now.");
   ...
   * Everything it needs comes from ONE extra call to the speaker: browseQueue
   * from the current position returns both what is still to come and how long
   * the queue is.
   *
   * The queue is read back off the PLAYER rather than remembered here, for the
   * same reason the queue screen does it: the Sonos app can add to it, remove
   * from it and reorder it, and a radio working from its own idea of the queue
   * would either stop early or pile albums onto one that was already full.
   */
  async topUp(coord, track, pos) {
    if (!this.radio || !this.radio.wanted()) return;
    ...
    const browsed = await coord.browseQueue(0, this.radio.MAX_QUEUE);
    ...
    const next = this.radio.pick({
      genre: this.radio.status().matchGenre && album ? album.genre || "" : "",
      exclude: [...played]
    });
    if (!next) return;
