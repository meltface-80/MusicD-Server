 ...
 *                    downmix this used to ask for reads an out-of-phase
 *                    passage as silence
 *   -ar 44100        see DECODE_RATE below
 *
 ...
 *
 * 44.1 kHz, WHICH IS FASTER THAN THE 16 kHz IT REPLACED. The old reasoning was
 * that the extra PCM had to be paid for somewhere; measured, the opposite is
 * true, because nearly every file in a library already IS 44.1 kHz and asking
 * for it means ffmpeg has nothing to resample. Timed over a five-minute FLAC:
 * 16 kHz 203-216ms, 32 kHz 205-224ms, 44.1 kHz 171-194ms, accumulation
 * included. The anti-alias filter costs more than the bytes it saves.
 *
 ...
 * level — measured against the same signal band-limited both ways, up to 4.9%
 * of full height on deliberately bright material. Small next to what the
 * reduction was costing, but it is free, so there is no argument for keeping it.
 *
 ...
 * used to claim — ffmpeg averages them. Measured on a stereo file whose
 * channels are inverted against each other: the mono downmix comes back at RMS
 * 0, dead silence, where the two channels together are RMS 2896. Any record
 * with a phase-flipped or heavily decorrelated passage reads quieter than it
 * is, and in the limit reads as nothing at all.
 *
 ...
 * milliseconds now holds twice as many samples, which is why the stride is
 * computed from this.
 *
...
function decodeWaveform(file, opts) {
  ...
  return new Promise((resolve) => {
    ...
    /* The level window in SAMPLES at this rate — stated here rather than baked
       into the accumulator, so moving the rate again cannot silently change
       how long a level is. */
    const acc = createPeaks({
      /* The level window in SAMPLES: ten milliseconds of FRAMES, times the
         channels in each one. Stated here rather than baked into the
         accumulator, so moving the rate or the channel count again cannot
         silently change how long a level is. */
...
module.exports = { decodeWaveform, lastDecodeError, ffmpegPath, args,
                   DECODE_RATE, DECODE_CHANNELS, DEFAULT_TIMEOUT_MS };
