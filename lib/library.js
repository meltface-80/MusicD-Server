 ...
 */
/*
 * `plays` ANSWERS FOR THE ROOMS THIS INSTALL ACTUALLY HAS, and is injected
 * because a file's playability is a fact about SPEAKERS and this file knows
 * about a database. It used to be the Sonos list, hard-coded, which badged
 * every Opus and DSD file in a house that owns no Sonos.
 *
 * The default says yes to everything: a caller that has not said which rooms
 * exist has told us nothing, and "I do not know" must not paint as "this will
 * not play".
 */
function album(db, id, { plays = () => true } = {}) {
    ...
    `SELECT id, title, artist, disc, no, duration, mime, bitdepth, samplerate, rel,
     ...
     ORDER BY disc, no, rel`).all(row.id).map(t => ({
      ...
      /* Decided HERE, once, and sent to the client. The album screen badges
         files that will not play and the queue builder drops them; when each
         ...
         then vanished on Play. Both sides ask a PLAYER now, through the same
         `plays()`, so they cannot drift again. */
      playable: plays({
        ext: path.extname(t.rel || "").toLowerCase(),
        mime: t.mime || ""
      })
    }));
