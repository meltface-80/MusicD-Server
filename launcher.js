...
// (Taken from MusicD Remote.)
//
// Copyright (c) 2026 Lewis Menzies (Music Duck / MusicD)
// Released under the MIT License. See the LICENSE file for details.
//
// The Docker image runs this (and so does `npm start`) to get one-tap updates
...
// Timestamp the launcher's own few lines the way index.js stamps its output.
for (const _level of ["log", "warn", "error"]) {
  const _orig = console[_level].bind(console);
  console[_level] = (...args) => _orig(new Date().toISOString().slice(11, 19), ...args);
}
...
function start() {
  const child = spawn(process.execPath, [path.join(DIR, "index.js")], {
    cwd: DIR,
    stdio: "inherit",
    env: Object.assign({}, process.env, { MUSICD_VIA_LAUNCHER: "1" })
  });
